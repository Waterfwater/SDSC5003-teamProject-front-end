<template>
    <div>
        ...
        <el-breadcrumb :separator-icon="ArrowRight">
            ...
            <el-breadcrumb-item>Duty Schedule List</el-breadcrumb-item> 
        </el-breadcrumb>
        ...
        <div class="page_content">
            ...
            <el-table class="table" :data="paginatedData" style="width: 100%">
                <el-table-column prop="schedule_date" label="Duty Date" />
                <el-table-column prop="schedule_time" label="Duty Time" />
            </el-table>
...
import { dutyListApi } from '@/util/request'
    ...
    setup(){
        ...
        const filter = () => {
            ...
            data.filteredUserList = data.UserList.filter(user => {//更改
                ...
                return (
                    (user.schedule_id && String(user.schedule_id).toLowerCase().includes(query)) ||
                    (user.schedule_date && user.schedule_date.toString().toLowerCase().includes(query)) ||
                    (user.schedule_time && user.schedule_time.toLowerCase().includes(query))
                    // 注意所有的字段现在都进行了存在性检查
                );
            });
            data.total = data.filteredUserList.length; // 更新总数据量
